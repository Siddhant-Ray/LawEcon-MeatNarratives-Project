...
class DataCleanerGerman:
    ...
    def generate_dataset(fil_dir):
        ...
        statements = [
            "instrument" if item.casefold() == "policy instrument" else item
            for item in statements
        ]
        statements = [
            "goal" if item.casefold() == "policy goal" else item for item in statements
        ]

        valence = final_df["topic_valence"].to_list()
        valence = [
            "contra" if item.casefold() == "contra meat" else item for item in valence
        ]
        valence = [
            "contra" if item.casefold() == "contra plant-based" else item
            for item in valence
        ]
        valence = [
            "pro" if item.casefold() == "pro plant-based" else item for item in valence
        ]
        ...
        references = [
            "economy" if item.casefold() == "economic" else item for item in references
        ]
        references = [
            "animal welfare" if item.casefold() == "ann" else item
            for item in references
        ]
        references = [
            "moral and ethic" if item.casefold() == "moral and ethics" else item
            for item in references
        ]
        references = [
            "water usage and quality"
            if item.casefold() == "water-usage and quality"
            else item
            for item in references
        ]
        ...
        final_df[label_cols] = final_df.apply(
            lambda row: row[label_cols].str.lower(), axis=1
        )
    ...
    def pre_process_data_to_numeric_labels(dataframe):
        ...
        dataframe["statement_type"] = dataframe.apply(
            lambda row: statement_type_dict[row["statement_type"]], axis=1
        )
        dataframe["statement_topic"] = dataframe.apply(
            lambda row: statement_topic_dict[row["statement_topic"]], axis=1
        )
        dataframe["topic_valence"] = dataframe.apply(
            lambda row: topic_valence_dict[row["topic_valence"]], axis=1
        )
        dataframe["statment_reference"] = dataframe.apply(
            lambda row: statement_reference_dict[row["statement_reference"]], axis=1
        )
...
class DataCleanerEnglish:
    ...
    def generate_dataset(fil_dir):
        ...
        statements = final_df["statement_type"].to_list()
        statements = [
            "narrative" if item.casefold() == "narrativ" else item
            for item in statements
        ]
        statements = [
            "instrument" if item.casefold() == "policy instrument" else item
        ...
        references = [
            "food security" if item.casefold() == "world food supply" else item
            for item in references
        ]
        references = [
            "social fairness" if item.casefold() == "social inequality" else item
            for item in references
        ]
    ...
    def __getitem__(self, idx):
        ...
        sample = {
            ...
            "labels": {
                "label_type": label0,
                "label_topic": label1,
                "label_valence": label2,
                "label_reference": label3,
            },
        }
...
class BERTMultilabelClassifier(nn.Module):
    ...
    def __init__(self, n_type, n_topic, n_valence, n_reference, language="en"):
        ...
        self.model = BertForSequenceClassification.from_pretrained(
            self.MODEL, num_labels=10
        )
        self.model.dropout = Identity()
        self.model.classifier = Identity()

        self.s_type = nn.Sequential(
            nn.Dropout(p=0.2), nn.Linear(in_features=768, out_features=n_type)
        )
        self.s_topic = nn.Sequential(
            nn.Dropout(p=0.2), nn.Linear(in_features=768, out_features=n_topic)
        )
        self.s_valence = nn.Sequential(
            nn.Dropout(p=0.2), nn.Linear(in_features=768, out_features=n_valence)
        )
        self.s_reference = nn.Sequential(
            nn.Dropout(p=0.2), nn.Linear(in_features=768, out_features=n_reference)
        )
    ...
    for epoch in tqdm(range(num_epochs)):
        ...
        for idx, batch in enumerate(train_loader):
            ...
            if (idx + 1) % (int(n_total_steps / 1)) == 0:
                ...
                print(
                    f"Epoch [{epoch+1}/{num_epochs}], Step [{idx+1}/{n_total_steps}], Loss: {checkpoint_loss:.4f}"
                )
...
def validate_model(model, device, val_loader, batch_size, model_type):
    ...
    with torch.no_grad():
        ...
        print(
            "Accuracy of statement topic is ", accuracy_dict["topic"] / len(val_loader)
        )
        print(
            "Accuracy of topic valence is ", accuracy_dict["valence"] / len(val_loader)
        )
        print(
            "Accuracy of statement reference is ",
            accuracy_dict["reference"] / len(val_loader),
        )
...
def run(args):
    ...
    if args.dataset == "german":
        ...
        (
            ...
            counter3,
            counter4,
        ) = DataCleanerGerman.count_label_occurrences(df)
        ...
        print(len(features))
        print([len(labels[key]) for key in labels.keys()])
        ...
        if args.model == "bert":
            ...
            tokenized_inputs = tokenizer(
                list_of_sentences, return_tensors="pt", truncation=True, padding=True
            )
        ...
        (
            ...
            counter3,
            counter4,
        ) = DataCleanerEnglish.count_label_occurrences(df)
        print(json.dumps(counter1, indent=2))
        print(json.dumps(counter2, indent=2))
        print(json.dumps(counter3, indent=2))
        print(json.dumps(counter4, indent=2))
    ...
    print(
        "Size for the target in our sample batch: {}".format(
            len(sample["labels"]["label_type"])
        )
    )
    print(
        "Targets for each batch in our sample: {}".format(
            sample["labels"]["label_type"]
        )
    )
    ...
    if args.dataset == "german":
        classifier = BERTMultilabelClassifier(
            num_types, num_topics, num_valence, num_refs, language="de"
        )
    elif args.dataset == "english":
        if args.model == "bert":
            classifier = BERTMultilabelClassifier(
                num_types, num_topics, num_valence, num_refs, language="en"
            )
        elif args.model == "roberta":
            classifier = RoBERTaMultilabelClassifier(
                num_types, num_topics, num_valence, num_refs, language="en"
            )
    # print(classifier)
    ...
    val_losses, accuracy_values, actual_values, predicted_values = validate_model(
        classifier, device, val_loader, args.batch_size, args.model
    )
    ...
    for key in actual_values.keys():
        if key != "valence":
            print(
                "F1 macro score on statement {} is ".format(key),
                calculate_f1_score(actual_values[key], predicted_values[key]),
            )
        else:
            print(
                "F1 macro score on topic {} is ".format(key),
                calculate_f1_score(actual_values[key], predicted_values[key]),
            )
...
def main():
    ...
    parser.add_argument(
        "-data", "--dataset", type=str, required=False, help="choose directory"
    )
    parser.add_argument("-model", "--model", type=str, help="choose model")
